...
class PlotType(Enum):
    ...
    def __contains__(cls, value: Self):
        if cls.match(value) is cls.NONE:
            return False
        return True
...
class LineEq(Equation):
    ...
    def _get_eq(self) -> None:
        self._equation = (
            rf"$y={self._slope:.4f} \cdot x + {self._intercept:.4f}$"
            + "\n"
...
class ExpEq(Equation):
    ...
    def _get_eq(self) -> None:
        self._equation = (
            rf"$y={self._intercept:.4f} \cdot x^{{ {self._slope:.4f} }}$"
            + "\n"
            + f"r: {self._r_value:.3f}\n"
        )
...
class LogEq(Equation):
    ...
    def _get_eq(self) -> None:
        self._equation = (
            rf"$y={self._intercept:.4f} \cdot \log_2({self._slope:.4f} \cdot x)$"
            + "\n"
            + f"r: {self._r_value:.3f}\n"
        )


class Plot(TypedDict, total=True):
    """
    ...
    type: PlotType
    size: int
    approximation: Equation | None


class Graph(TypedDict, total=True):
    """
